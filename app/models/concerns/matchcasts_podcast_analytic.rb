module MatchcastsPodcastAnalytic
  extend ActiveSupport::Concern

  def matchcasts_podcast_analytic
    filters = Utils.generate_filters()

    rollups = Rollup.where_dimensions(podcast_id: self.id).multi_series("Podcast listener")
    unique_rollups = Rollup.where_dimensions(podcast_id: self.id).multi_series("Podcast unique listener")
