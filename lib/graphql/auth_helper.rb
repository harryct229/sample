...
module Graphql
  module AuthHelper
    ...
    def current_user
      ...
      issued_at = decrypted_token.try(:[], 'issued_at')
      raise JWT::ExpiredSignature if issued_at.blank?
      raise JWT::ExpiredSignature if user.password_changed_at && Time.at(issued_at) < user.password_changed_at
